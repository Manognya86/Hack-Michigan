...
# ============================================================
# 1. REAL UTILITY POLE DATA FROM OPENSTREETMAP (Expanded Query)
# ============================================================
...
def get_poles_from_osm(lat: float, lon: float, radius_km: float = 5.0) -> list:
    ...
    Expanded query includes power=pole, power=tower, man_made=utility_pole,
    and highway=street_lamp (common tags for poles).
    ...
    # Expanded query: more tags for poles
    query = f"""
    [out:json];
    (
      node["power"="pole"](around:{radius_m},{lat},{lon});
      node["power"="tower"](around:{radius_m},{lat},{lon});
      node["man_made"="utility_pole"](around:{radius_m},{lat},{lon});
    ...
    try:
        ...
        for element in data.get('elements', []):
            ...
            pole = {
                ...
                'lat': element.get('lat'),
                'lng': element.get('lon'),
                'power_type': tags.get('power') or tags.get('man_made') or tags.get('highway', 'pole'),
        ...
        logger.info(f"Fetched {len(poles)} poles from OSM (expanded query)")
        return poles
