...
def insert_sample_history():
    ...
    conn.commit()
    conn.close()
    logging.info("Sample history inserted.")
...
def train_cost_model():
    ...
    conn.close()
    if df.empty:
        logging.warning("No historical data; dummy model")
        return None
    ...
    logging.info("Cost model trained.")
    return model
...
def predict_repair_cost(damage_severity, wind_speed, rainfall, pole_age, material):
    model = load_cost_model()
    if model is None:
        return damage_severity*20 + wind_speed*3 + rainfall*80
    material_code = {'wood':0, 'steel':1, 'composite':2}.get(material.lower(), 0)
    features = np.array([[damage_severity, wind_speed, rainfall, pole_age, material_code]])
