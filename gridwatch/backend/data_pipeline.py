...
Now assigns real districts based on lat/lng bounds (no external geocoding).
"""
...
# DTE service territory – actual district bounds (approximate)
DISTRICT_BOUNDS = [
    (42.330, 42.350, -83.120, -83.080, "Southwest Detroit"),
    (42.300, 42.340, -83.200, -83.140, "Dearborn"),
    (42.250, 42.300, -83.100, -83.020, "Downriver"),
    (42.360, 42.400, -83.400, -83.280, "Livonia"),
    (42.240, 42.280, -83.250, -83.180, "Allen Park"),
    (42.200, 42.240, -83.180, -83.120, "Wyandotte"),
    (42.380, 42.420, -83.100, -83.020, "Hamtramck"),
    (42.280, 42.320, -83.350, -83.280, "Inkster"),
    (42.230, 42.270, -83.200, -83.140, "Lincoln Park"),
    (42.180, 42.220, -83.180, -83.120, "Wyandotte South"),
    (42.220, 42.260, -83.320, -83.240, "Taylor"),
    (42.260, 42.300, -83.220, -83.160, "Melvindale"),
]
DEFAULT_DISTRICT = "Metro Detroit"

def lat_lng_to_district(lat: float, lng: float) -> str:
    """Return district name based on latitude/longitude bounds."""
    for lat_min, lat_max, lng_min, lng_max, district in DISTRICT_BOUNDS:
...
async def fetch_aqi(lat: float, lng: float) -> int:
    """Simulated Air Quality Index (0-500)."""
...
def _weather_desc(code: int) -> str:
    codes = {0:"Clear",1:"Mainly clear",2:"Partly cloudy",3:"Overcast",45:"Fog",51:"Drizzle",61:"Rain",80:"Showers",95:"Thunderstorm"}
...
async def fetch_osm_poles(limit: int = 150) -> List[Dict]:
    ...
    b = {"lat_min": 42.15, "lat_max": 42.55, "lng_min": -83.45, "lng_max": -82.85}
        ...
        async with httpx.AsyncClient(timeout=45.0) as client:
            ...
            for i, el in enumerate(elements):
                ...
                if lat is None or lng is None:
                    continue
                district = lat_lng_to_district(lat, lng)  # <-- real district mapping
                ...
                poles.append({
                    ...
                    "age": age, "material": material, "tilt_angle": round(tilt,1), "crack_detected": crack,
                    "rust_detected": rust, "vegetation_risk": veg, "wind_exposure": wind_exp,
                    "flood_zone": flood, "soil_type": soil, "storm_exposure_index": round(storm_exp,3),
                    "years_since_inspection": round(years_since,2), "urban_density": random.uniform(0.4,0.9),
                    "last_inspection": last_insp, "source": "osm-live",
                    "height_ft": height, "road_proximity_ft": road, "num_transformers": xfmr,
                    "last_maintenance_year": last_maint,
                    "aqi": aqi, "soil_moisture": 0.5  # placeholder, will be updated later
                })
            logger.info(f"Fetched {len(poles)} live OSM poles with real districts")
            return poles
    except Exception as e:
...
async def _synthetic_poles(n: int) -> List[Dict]:
    """Generate realistic synthetic poles as fallback (with district mapping)."""
    poles = []
    for i in range(n):
        lat = random.uniform(42.15, 42.55)
        lng = random.uniform(-83.45, -82.85)
        district = lat_lng_to_district(lat, lng)
        age = int(np.clip(np.random.normal(35, 12), 1, 70))
        material = random.choices(["Wood","Steel","Concrete","Composite"], weights=[0.6,0.22,0.1,0.08])[0]
        tilt = float(np.clip(np.random.normal(age*0.2, 3), 0, 25))
        crack = int(random.random() < (0.03 + age*0.008))
        ...
        poles.append({
            ...
            "flood_zone": flood, "soil_type": soil, "storm_exposure_index": round(storm_exp,3),
            "years_since_inspection": round(years_since,2), "urban_density": random.uniform(0.4,0.9),
            "last_inspection": last_insp, "source": "synthetic",
            "height_ft": height, "road_proximity_ft": road, "num_transformers": xfmr,
            "last_maintenance_year": last_maint,
            "aqi": aqi, "soil_moisture": 0.5
        })
    return poles
